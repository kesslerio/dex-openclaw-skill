...
sys.path.insert(0, "/home/art/niemand/skills/oura-analytics/scripts")
...
def send_telegram(message: str) -> bool:
    ...
    token = os.environ.get("TELEGRAM_BOT_TOKEN") or os.environ.get("KESSLER_TELEGRAM_BOT_TOKEN")
...
def generate_report(date_str: str = None, baseline_days: int = 30) -> str:
    ...
    # Default to yesterday (Oura data lags by a day)
    if date_str:
    ...
    if readiness_score:
        ...
        if negative_drivers:
            ...
            for driver in negative_drivers[:3]:
                lines.append(f"      • {driver.name}: {driver.actual:.0f} (baseline: {driver.expected:.0f})")
...
def main():
    ...
    parser.add_argument("--no-send", action="store_true", help="Print only, don't send to Telegram")
    ...
    try:
        ...
        if args.no_send:
            ...
            if send_telegram(report):
                print(f"✅ Report sent successfully at {datetime.now().strftime('%H:%M')}")
            else:
                print("❌ Failed to send report")
                print(report)  # Print for debugging
                sys.exit(1)
